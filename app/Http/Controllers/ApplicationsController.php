...
class ApplicationsController extends Controller
{
    ...
    public function email_marketing_getapplications(Request $request)
    {
        $usr = \Auth::user();
        ...
        $app_query->where('contact_details.is_email_bogus', 0);

        $app_query->whereNotNull('contact_details.contact_email');
    ...
    public function addApplicationMeta(Request $request)
    {
        ...
        // Get application
        $application = DealApplication::with(['brand', 'branch', 'coursedetail', 'deal'])->find($request->application_id);
        ...
        if ($request->stage_id == 11) {
            \DB::table('finance_student_files')->updateOrInsert(
                ...
                [
                    ...
                    'admission_officer_id' => $application->deal->assigned_to ?? 0,
